...
/* Modification Record                                                        */
/* Date     Inits   Comments                                                  */
/* 26/02/16 GMM     Modified for Autogrid flags:                              */
/*                  -p = Parameter filename;                                  */
/*                  -l = Log filename;                                        */
/*                  -o = Use old PDBq format (q in columns 55-61)             */
/*                  -w = Water Sites file                                     */
...
/*----------------------------------------------------------------------------*/
    while((argc > 1) && (argv[1][0] == '-')){
        ...
        case 'w':
         if (argc < 3){
                fprintf(stderr, "\n%s: Sorry, -w requires a .dat filename.\n\t%s\n", programname, AutoGridHelp);
                exit(EXIT_FAILURE);
            }
            //Parameter retrival was passed to function at main.cpp file due to problems with global variable's declaration
            argv++;
            argc--;
            argindex++;
            break;
