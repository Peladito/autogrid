...
//El parseo reconoce como token de ceparacion al TAB ("\t")
    double get_data_from_pos(std::string s, int posicion){
      ...
      return std::atof(s.substr(last_pos, pos).c_str());
    }
  //La siguiente funcion recive el argc y argv del main y recupera el parámetro -w el cual no lo pude recuperar en una global desde setflags (work around)
    ...
    char* get_w_parameter( int argc, char **argv){
...
//Una posible mejora es dejar de considerar un cubo y pasar a una esfer (slo guardariamos el centro de la esfera y su energia)
          typedef struct cubo {
            double x_min, x_max, y_min, y_max, z_min, z_max, xmax, ymin, zc, e5, e6;
          } Cubo;
...
//Se defina la estructura principal: en ella se van a guardar: el radio de los cubos, la cantidad de WS, la resolucion de la grilla y el centro de la grilla en coordenadas de la grilla
    ...
    */
    gridTransformer( double cx, double cy, double cz, double resol, char* filename): last_cubo(0), resolucion(resol){
      std::string line;
       ...
       if (myfile.is_open())
       {
        while ( getline (myfile,line) )
        {
          if (!header){
            ...
            cantWs = atoi(line.c_str());
            printf("%s\n","Creando GridTransformer:");
            centro_grilla_x = double(int(cx / resol));
            centro_grilla_y = double(int(cy / resol));
            centro_grilla_z = double(int(cz / resol)); 

            printf("%s centro grilla = (%f, %f, %f)\n","[WS] GridTransformer inicializado: ",centro_grilla_x, centro_grilla_y, centro_grilla_z);
  ...
  // printf("%s\n","[WS] Agregando WS al GridTransformer" );
         //Obtengo las coordenadas del agua y sus energias
     double x=get_data_from_pos(s,1);
     double y=get_data_from_pos(s,2);
     double z=get_data_from_pos(s,3);
     double e5=get_data_from_pos(s,5);
     double e6=get_data_from_pos(s,6);
     ...
     double z_max = nz + radio - centro_grilla_z;
        ...
        //listWs[last_cubo] = Cubo(x_min, x_max,x, y_min, y_max,y, z_min, z_max,z, e5, e6);
     ...
     listWs[last_cubo].e5 = e5;
     listWs[last_cubo].e6 = e6;
   ...
   cubo* in_ws(double x, double y, double z){
        // printf("%s\n","Evaluando pertenencia" );
    int i = 0;
    for (i = 0; i < cantWs; i++)
    {
      if (x >= listWs[i].x_min && x <= listWs[i].xmax && y >= listWs[i].ymin && y <= listWs[i].y_max && z >= listWs[i].z_min && z <= listWs[i].z_max)
      {
        break; 
      }
    }
  ...
  double modify(double x, double y, double z, double e){
        //print_error( logFile, WARNING, message );
    double em = e;
    Cubo* cubo = in_ws(x, y, z);
    if (cubo != NULL)
    {
...
//No funciona  la gausiana pq es gato y cobra
        em = e + cubo->e5 ;//* exp(-(dist/cubo->e6));

        printf("[WS MOD]:  e_i = %f, e_f = %f, dist a WS = %f , e5 = %f, e6 = %f : e_f - e_i = %f \n", e, em, dist, cubo->e5,cubo->e6, em - e /*(cubo->e5 * exp(-(dist/cubo->e6)))*/ );
      }else{
            //printf("out");
...
/*      Date: 07/07/04                                                        */
/*                                                                            */
/*    Inputs: Control file, receptor PDBQT file, parameter file, ws file      */
 ...
 */
 ...
 char* WSF = get_w_parameter(argc, argv);
...
//Carlos
//no me esta reconociendo la variable global WSF del archivo setflags por donde viene el parametro
//std::ifstream *WSF;
//WSF->open("/home/cpmode/Dropbox/glyco-division/Esteban/mete_pozo/biasM_M-BSCWS.dat");
//GridTransformer gt_carlos (center[X], center[Y], center[Z], spacing, "/home/esteban/Dropbox/glyco-division/Esteban/mete_pozo/biasM_M-BSCWS.dat");
GridTransformer gt_carlos (center[X], center[Y], center[Z], spacing, WSF);
...
/* END NEW2 calculate dot product of bond vector with bond vector of best hbond */
            ...
            //----------------------------------------------------------------------------------------------
            //-----------------Modificacion de la energia de carlos's potential-----------------------------
            //----------------------------------------------------------------------------------------------

                    //WSF+=1;
                double ener = gt_carlos.modify(icoord[X], icoord[Y], icoord[Z], gridmap[map_index].energy);
                gridmap[map_index].energy = ener;
